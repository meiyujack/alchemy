...
class BookController(Controller):
    ...
    async def list_books(self,books_repo:BookRepository,limit_offset:filters.LimitOffset)->OffsetPagination[BookRead]:
        ...
        books=[BookRead(book.title,book.id) for book in results]
        return OffsetPagination[BookRead](
            items=books,#type_adapter.validate_python(results),
            total=total,
    ...
    @post(path="/books")
    async def create_book(self,books_repo:BookRepository,data:BookCreate)->BookRead:
        obj=await books_repo.add(Book(title=data.title,author_id=data.author_id))#**data.model_dump(exclude_unset=True,exclude_none=True)))
        await books_repo.session.commit()
        return BookRead(obj.title,obj.id)#BookRead.model_validate(obj)
    ...
    async def update_book(self,books_repo:BookRepository,data:BookUpdate,book_id:UUID=Parameter(title="Book ID",description="The book to update."))->BookRead:
        raw_obj=data.__dict__#data.model_dump(exclude_unset=True,exclude_none=True)
